...
.env: 為了安全性，敏感資訊（如 API Keys）不會上傳到版本控制。請根據此範本建立 .env 檔案。
...
此執行檔已經預設配置了**容錯機制**。它會先嘗試使用 **Gemini 2.0 Flash** 分析圖片；
如果 Gemini 發生錯誤或配額耗盡，系統會自動切換使用 **GitHub Models 的 GPT-4o**；若再次失敗，
則會使用 **GPT-4o-mini** 進行最後嘗試。這確保了服務的高度穩定性。
...
.env: For security purposes, sensitive information (like API Keys) is not tracked by version control. Please copy this file and rename it to .env to configure your settings.
...
1. Install the required dependencies:
   ...
   pip install -r requirements.txt
   ```
2. Copy and rename to .env, then fill in the following information:
   ```env
   # Google AI Studio (Gemini)
